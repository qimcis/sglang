...
class LatentUpsamplerLoader(ComponentLoader):
    component_names = ["latent_upsampler"]
    ...
    def load_customized(
        ...
        if cls_name is None:
            raise ValueError(
                ...
                "Only diffusers-style component folders are supported."
            )
        ...
        init_kwargs = {k: v for k, v in config.items() if k in init_sig.parameters}

        with set_default_torch_dtype(default_dtype), skip_init_modules():
            model = model_cls(**init_kwargs).to(
