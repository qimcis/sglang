...
def plan_topk_v2(seq_lens: torch.Tensor, static_threshold: int = 0) -> torch.Tensor:
    ...
    Producers should represent padded rows with length 0. The device kernel
    also clamps negative and over-capacity values as a safety backstop.
    """
...
def topk_transform_512_v2(
    ...
    values. A length of 0 expresses an inactive row and produces all -1.
    """
    module = _jit_topk_v2_module()
    module.topk_transform(
        scores,
