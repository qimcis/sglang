...
- Approach: the bottom registry declares KVCacheAttentionAccess entries for
  each recipe. A backend resolves one entry by (phase, backend_name),
...
class KVCacheBackendMatcher:
    ...
    def matches(self, backend_name: str) -> bool:
        return self.any_backend or backend_name in self.exact
...
class KVCacheAttentionAccess:
    ...
    - backend_matcher: backend names that select this rule, e.g.
      exact={"trtllm_mha"}, or a matcher that accepts any backend.
    ...
    def matches(self, phase, backend_name: str) -> bool:
        return self.phase == KVCacheAttentionPhase(
            phase
        ) and self.backend_matcher.matches(backend_name)
...
class KVCacheQuantMethodBase(ABC):
    ...
    def describe_attention_accesses(self, phase=None) -> str:
        ...
        return "; ".join(
            ...
            f"any_backend={access.backend_matcher.any_backend}"
    ...
    def quantize_dequantize_current_kv(
        self, cache_k: Tensor, cache_v: Tensor, layer_id: int
    ) -> tuple[Tensor, Tensor]:
        """Apply the storage codec before current K/V enters a dequant workspace."""
        raise NotImplementedError(
            f"KV cache method {self.name!r} does not implement current-KV roundtrip"
        )
    ...
    def get_scale_rows(
        ...
        *,
        ...
        """Gather scale rows for token-slot indices."""
        return scale_buffer[loc]
    ...
    def set_scale_rows(
        ...
        *,
        ...
        """Scatter scale rows for token-slot indices."""
        scale_buffer[loc] = rows

    def move_scale_rows(
        self,
        k_scale_buffer: Tensor,
        v_scale_buffer: Tensor,
        tgt_loc: Tensor,
        src_loc: Tensor,
        page_size: int,
    ) -> None:
        """Move K/V scale rows between token slots."""
        k_rows = self.get_scale_rows(k_scale_buffer, src_loc, page_size).clone()
        v_rows = self.get_scale_rows(
            v_scale_buffer, src_loc, page_size, is_value=True
        ).clone()
        self.set_scale_rows(k_scale_buffer, tgt_loc, k_rows, page_size)
        self.set_scale_rows(v_scale_buffer, tgt_loc, v_rows, page_size, is_value=True)
...
class UnquantizedKVCacheMethod(KVCacheQuantMethodBase):
    ...
    def create_buffers(
        self, size, head_num, head_dim, layer_num, device, page_size=1
    ) -> dict:
        pass
...
class NVFP4KVCacheMethod(KVCacheQuantMethodBase):
    ...
    def __init__(self, num_layers: int, device: str):
        if torch.device(device).type == "cuda":
            try:
                ...
                raise RuntimeError(
                    "NVFP4 KV cache requires the paired FlashInfer build with "
                    "nvfp4_quantize_append_paged_kv_cache_with_slot_mapping"
                ) from exc
            if not callable(nvfp4_quantize_append_paged_kv_cache_with_slot_mapping):
                raise RuntimeError(
                    "The paired FlashInfer NVFP4 slot-mapping API is not callable"
                )
    ...
    def _paged_scale_indices(
        self, scale_buffer: Tensor, loc: Tensor, page_size: int, is_value: bool
    ) -> tuple[Tensor, Optional[Tensor]]:
        pages = torch.div(loc, page_size, rounding_mode="floor")
        offsets = torch.remainder(loc, page_size)
        if not is_value or not self._interleave_v_scales:
            return pages, offsets

        blocks = scale_buffer.shape[-1]
        block_offsets = torch.arange(blocks, device=loc.device)
        physical_offsets = (
            torch.div(offsets[:, None], 4, rounding_mode="floor") * (4 * blocks)
            + block_offsets[None, :] * 4
            + torch.remainder(offsets[:, None], 4)
        )
        return pages, physical_offsets
    ...
    def get_scale_rows(
        ...
        *,
        ...
        pages, offsets = self._paged_scale_indices(
            scale_buffer, loc, page_size, is_value
        )
        if offsets.ndim == 1:
            return scale_buffer[pages, :, offsets, :]

        page_rows = scale_buffer.flatten(2).index_select(0, pages)
        gather_indices = offsets[:, None, :].expand(-1, page_rows.shape[1], -1)
        return torch.gather(page_rows, 2, gather_indices)
    ...
    def set_scale_rows(
        ...
        *,
        ...
        if offsets.ndim == 1:
            scale_buffer[pages, :, offsets, :] = rows
            return

        heads = torch.arange(scale_buffer.shape[1], device=loc.device)
        scale_buffer.flatten(2)[
            pages[:, None, None], heads[None, :, None], offsets[:, None, :]
        ] = rows
    ...
    def load_scales_from_model(self, model_runner) -> None:
        ...
        for layer in attention_layers:
            ...
            if is_sm100_supported() and getattr(
                layer, "_kv_scales_loaded_from_checkpoint", True
            ):
                k_scale *= E2M1_MAX
                v_scale *= E2M1_MAX
    ...
    def create_buffers(
        ...
        if page_size <= 0 or m % page_size != 0:
            raise ValueError(
                f"NVFP4 scale storage requires size divisible by page_size, got "
                f"size={m}, page_size={page_size}."
            )
        num_pages = m // page_size
        # Stock TRTLLM-Gen interleaves V scales in four-token groups. The H512
        # candidate cubin deliberately consumes linear V scales, matching K.
        self._interleave_v_scales = k != 512
        if self._interleave_v_scales and page_size % 4 != 0:
            raise ValueError(
                "TRTLLM-Gen NVFP4 V-scale storage requires page_size divisible "
                f"by 4, got page_size={page_size}."
            )
        ...
        k_scale_buffer = [
            torch.zeros(
                (num_pages, n, page_size, k // self.SCALE_BLOCK_SIZE),
                dtype=store_dtype,
                device=device,
            )
            for _ in range(layer_num)
    ...
    def quantize_and_store(
        ...
        k_scale=None,
        v_scale=None,
        fused_q_src: Optional[Tensor] = None,
        fused_q_dst: Optional[Tensor] = None,
        fused_context_dst_rows: Optional[Tensor] = None,
        fused_context_k_dst: Optional[Tensor] = None,
        fused_context_v_dst: Optional[Tensor] = None,
    ) -> None:
        # H256 interleaves V scales; K scales and H512 scales remain linear.
        # Explicit row strides avoid requiring contiguous QKV slices.
        head_dim = cache_k.shape[-1]
        if not (
            (head_dim == 256 and self._interleave_v_scales)
            or (head_dim == 512 and not self._interleave_v_scales)
        ):
            raise RuntimeError(
                "unsupported native NVFP4 KV layout: "
                f"head_dim={head_dim}, "
                f"interleave_v_scales={self._interleave_v_scales}"
            )
        for name, rows in (("cache_k", cache_k), ("cache_v", cache_v)):
            if rows.dim() != 3 or rows.stride(-1) != 1:
                raise RuntimeError(
                    "fused NVFP4 store requires 3-D K/V rows with a "
                    f"contiguous last dim; got {name} "
                    f"shape={tuple(rows.shape)} stride={rows.stride()}"
                )
        if (fused_q_src is None) != (fused_q_dst is None):
            raise RuntimeError("fused_q_src and fused_q_dst must be provided together")
        if fused_q_src is not None and fused_q_src.shape[0] != loc.shape[0]:
            # The writer casts exactly loc.shape[0] rows; a larger Q would
            # leave uncast tail rows feeding attention, a smaller one is a
            # caller bug. Decode always emits one slot per token, so demand
            # exact agreement.
            raise RuntimeError(
                "fused Q cast requires one Q row per cache slot; got "
                f"q rows={fused_q_src.shape[0]} slots={loc.shape[0]}"
            )

        # Context rows address P16 scratch independently of persistent cache slots.
        context_args = (
            fused_context_dst_rows,
            fused_context_k_dst,
            fused_context_v_dst,
        )
        has_context = fused_context_dst_rows is not None
        if any(value is not None for value in context_args) != all(
            value is not None for value in context_args
        ):
            raise RuntimeError(
                "fused_context_dst_rows, fused_context_k_dst, and "
                "fused_context_v_dst must be provided together"
            )
        if has_context:
            if head_dim != 256 or not self._interleave_v_scales:
                raise RuntimeError(
                    "fused context outputs require the H256 interleaved NVFP4 route"
                )
            if (
                fused_context_dst_rows.dim() != 1
                or fused_context_dst_rows.shape[0] != loc.shape[0]
            ):
                raise RuntimeError(
                    "fused context destinations require one row per cache slot"
                )
            expected_context_tail = (cache_k.shape[1], head_dim)
            if (
                fused_context_k_dst.dim() != 3
                or fused_context_v_dst.shape != fused_context_k_dst.shape
                or fused_context_k_dst.shape[1:] != expected_context_tail
            ):
                raise RuntimeError(
                    "fused context K/V outputs must match the source K/V head shape"
                )
        ...
        page_size = k_scale_buffer.shape[2]
        num_pages = k_scale_buffer.shape[0]
        num_heads = k_buffer.shape[1]
        packed_head_dim = k_buffer.shape[2]
        paged_k = k_buffer.view(
            num_pages, page_size, num_heads, packed_head_dim
        ).permute(0, 2, 1, 3)
        paged_v = v_buffer.view(
            num_pages, page_size, num_heads, packed_head_dim
        ).permute(0, 2, 1, 3)
        nvfp4_quantize_append_paged_kv_cache_with_slot_mapping(
            cache_k,
            ...
            (paged_k, paged_v),
            (
                k_scale_buffer.view(torch.float8_e4m3fn),
                v_scale_buffer.view(torch.float8_e4m3fn),
            ),
            k_scale,
            v_scale,
            kv_layout="HND",
            q_input=fused_q_src,
            q_output=fused_q_dst,
            context_dst_rows=fused_context_dst_rows,
            context_k_output=fused_context_k_dst,
            context_v_output=fused_context_v_dst,
        )

    def quantize_dequantize_current_kv(
        self, cache_k: Tensor, cache_v: Tensor, layer_id: int
    ) -> tuple[Tensor, Tensor]:
        """Match current prefill K/V precision to cached-prefix K/V precision."""
        from sglang.srt.layers.quantization.kvfp4_tensor import NVFP4KVQuantizeUtil

        k_scale = self.k_scales_gpu[layer_id : layer_id + 1]
        v_scale = self.v_scales_gpu[layer_id : layer_id + 1]
        cache_k_fp4, cache_k_fp4_sf, _ = NVFP4KVQuantizeUtil.quantize(
            cache_k.contiguous(), k_scale
        )
        cache_v_fp4, cache_v_fp4_sf, _ = NVFP4KVQuantizeUtil.quantize(
            cache_v.contiguous(), v_scale
        )
        return self.dequantize_prev_kv(
            cache_k_fp4.view(torch.uint8),
            cache_k_fp4_sf,
            cache_v_fp4.view(torch.uint8),
            cache_v_fp4_sf,
            layer_id,
        )
...
_NVFP4_DECODE_WORKSPACE_BACKENDS = frozenset({"flashinfer"})
_NVFP4_DECODE_NATIVE_BACKENDS = frozenset({"trtllm_mha"})
...
KV_CACHE_ATTENTION_ACCESS_REGISTRY: dict[str, tuple[KVCacheAttentionAccess, ...]] = {
    ...
    NVFP4KVCacheMethod.name: (
        ...
        _dq_workspace(
            _DECODE,
            _NVFP4_DECODE_WORKSPACE_BACKENDS,
            _NVFP4_SCALE,
            _FP8_E4M3,
        ),
        _native_fp4(_DECODE, _NVFP4_DECODE_NATIVE_BACKENDS, _NVFP4_SCALE, _TORCH_FP4),
    ),
