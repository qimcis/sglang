...
class SchedulerBatchResultProcessor:
    ...
    def _resolve_spec_v2_tokens(
        ...
        result.num_block_accept_tokens = (
            sum(
                count
                for req, count in zip(batch.reqs, block_accept_lens, strict=True)
                if req.rid not in failed_rids
            )
            if block_accept_lens
            else 0
        )
        cap_lens = result.cap_lens.tolist() if result.cap_lens is not None else None
        result.num_cap_tokens = (
            sum(
                count
                for req, count in zip(batch.reqs, cap_lens, strict=True)
                if req.rid not in failed_rids
            )
            if cap_lens
            else 0
