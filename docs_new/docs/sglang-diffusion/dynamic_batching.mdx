...
Admission learns from completed batches by default and caps future batches when the observed peak memory suggests the next batch would exceed available VRAM. This is an adaptive guard below `--batching-max-size` and any `--batching-config` rules, not a byte-exact startup predictor.

`--batching-memory-reserve-gb` controls how much device memory to leave unused. Prediction headroom is adaptive: successful batches compare measured peak memory to the predicted peak and store recent prediction error. `--batching-memory-safety-factor` is only the minimum multiplier floor for that learned headroom; the default floor is `1.0`. Memory profiles are scoped by model/runtime settings, precision, quantization, attention backend, parallelism, offload mode, LoRA revision, text-layout settings, and request shape. A new memory profile ramps geometrically from singleton execution to batch size two, then four, then eight, while using observed peaks, OOM records, interpolation within the observed cost range, and linear extrapolation for larger batches. Successful calibration profiles are cached under `${SGLANG_DIFFUSION_BATCH_MEMORY_CACHE:-~/.cache/sglang/diffusion_batch_memory}` using `schema_version: 1`.
...
- There is no startup probing, OOM retry, or automatic fallback to singletons. Admission starts each new memory profile with a singleton ramp and uses completed batch peak memory and recorded OOMs to cap future batches, but it does not retry a failed merged batch.
