...
BASE_URL=http://127.0.0.1:30210
MODEL=black-forest-labs/FLUX.2-klein-4B
RESULT_DIR=/workspace/continuous-bench/results/flux_klein_small
LOG_DIR=/workspace/continuous-bench/logs/flux_klein_small
...
start_server() {
  ...
  echo "=== starting FLUX Klein mode=${mode_name} batching_mode=${batching_mode} max=${max_size} delay=${delay_ms} ==="
  cleanup_server
  ...
  echo $! > /workspace/continuous-bench/flux_server.pid
  for i in $(seq 1 180); do
    if curl -fsS "$BASE_URL/health" >/dev/null 2>&1; then echo "server ${mode_name} ready"; return 0; fi
    if ! kill -0 "$(cat /workspace/continuous-bench/flux_server.pid)" 2>/dev/null; then echo "server ${mode_name} exited"; tail -200 "$LOG_DIR/server_${mode_name}.log"; return 1; fi
...
run_case() {
  ...
  echo "=== bench FLUX Klein ${mode_name} ${width}x${height} run${run} ==="
...
echo "=== flux klein small matrix complete ==="
