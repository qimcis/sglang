...
@pytest.mark.skipif(_SKIP, reason="fused FA3 protection requires Hopper")
def test_dsa_producer_sanitizes_before_unprotected_fa3():
    batch_size = 2
    seqlen = 2048
    page_size = 64
    pages_per_request = seqlen // page_size
    logical = torch.arange(seqlen, device="cuda", dtype=torch.int32)
    page_table = torch.stack(
        [
            (logical // page_size + 1 + row * pages_per_request) * page_size
            + logical % page_size
            for row in range(batch_size)
        ]
    ).contiguous()
    score = torch.zeros(batch_size, seqlen, device="cuda", dtype=torch.float32)
    seqlens = torch.full((batch_size,), seqlen, device="cuda", dtype=torch.int32)
    ...
    protection = {
        ...
        "actual_tags": page_ids + 1,
        "actual_generations": page_ids + 2,
        "actual_transfer_tags": (page_ids + 3).to(torch.int32),
        "owner_request_indices": owners,
        "owner_page_positions": positions,
        "expected_tags": page_ids + 1,
        "expected_generations": page_ids + 2,
        "expected_transfer_tags": (page_ids + 3).to(torch.int32),
        "request_epochs": torch.tensor([0, 1, 1], device="cuda", dtype=torch.int32),
        "validated_epochs": torch.full((3,), -1, device="cuda", dtype=torch.int32),
        "status": torch.zeros(3, device="cuda", dtype=torch.int32),
    }

    page_table[1, 62] += 1
    sparse_table = fast_topk_transform_fused(
        score,
        seqlens,
        page_table,
        cu_seqlens_q,
    ...
    assert protection["status"].tolist() == [0, 0, 1]
    assert protection["validated_epochs"].tolist() == [-1, 1, 1]
    assert torch.any(sparse_table[1] == 0)

    num_token_slots = batch_size * pages_per_request * page_size + page_size
    q = torch.randn(batch_size, 1, 8, 64, device="cuda", dtype=torch.bfloat16)
    qv = torch.randn(batch_size, 1, 8, 256, device="cuda", dtype=torch.bfloat16)
    k_cache = torch.randn(
        num_token_slots, 1, 1, 64, device="cuda", dtype=torch.bfloat16
    )
    v_cache = torch.randn(
        num_token_slots, 1, 1, 256, device="cuda", dtype=torch.bfloat16
    )
    result = flash_attn_with_kvcache(
        q=q,
        qv=qv,
